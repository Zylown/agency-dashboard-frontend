import { MdOutlineDashboard, MdTaskAlt } from "react-icons/md";
...
const iconandtext = [
  ...
  { icon: <LuUsers />, text: "Creadores" },
  { icon: <FaArrowTrendUp />, text: "Subscriptores" },
  { icon: <FaRegCalendar />, text: "Contenido" },
  { icon: <MdTaskAlt />, text: "Tareas" },
  { icon: <IoMdAnalytics />, text: "Analiticas" },
  { icon: <CiChat1 />, text: "Chatters" },
  { icon: <AiOutlineTeam />, text: "Team" },
...
export default function Home() {
  return (
    ...
    <div className="w-full max-w-60 h-screen border-r-2 border-[#f0f1f1]">
      ...
      <div className="p-6 pb-48 border-b-2 border-[#f0f1f1]">
        <ul className="flex flex-col gap-4 text-lg">
          {iconandtext.map((item, index) => (
            <li
              key={index}
              className="flex items-center gap-2 text-[#4a4a4a] font-medium"
            >
              {item.icon}
              <p>{item.text}</p>
            </li>
      ...
      <div className="flex px-6 pt-4 items-center justify-center gap-4">
        <div>
          <Image
            src={
